...
function spinalCase(str) {
    var spinalCase;
    if (str.indexOf(' ') != -1) {
        if ((str.match(/\s/g) || []).length === 1) {
            var mid = str.replace(/([A-Z])/g, '-$1');
            var removeFirst = mid.replace('-','');
            var removeSpace = removeFirst.replace(' ', '');
            //console.log("mid: "+mid);
            //console.log("no first -: "+ removeSpace.toLowerCase());
            spinalCase = removeSpace.toLowerCase();
        } else {
